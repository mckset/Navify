...
def SetupSettings():
	...
	try:
		...
		window = sg.Window("ERROR", 
		  layout = [[sg.Text("Youtube-dl is not installed. Please install Youtube-dl", text_color="#ff0000", background_color="#000000",  expand_x=True)]], 
		   resizable=True,
		   alpha_channel=1, 
		   background_color="#000000",
		   font=defaultFont,
	...
	if not exists(home + "playlists"):
		subprocess.run(["mkdir", home + "playlists"])
	...
	if not exists(playerLoc + "/Music"):
		print("WARNING: /home/user/Music does not exist. Navify can only browser files in the Music folder") 
...
def layout(queue, current, like):
	...
	BottomBar = [
		[
		sg.Text(text="Now Playing: " + current, size=(1,1), justification="left", font=smallFont, expand_x=True, background_color=foreground, key="-PLAYING-"),
		...
		sg.Button(image_filename=home + "icons/" + like, enable_events=True, mouseover_colors=hover, border_width=0, key="-LIKE-"),
		...
		if event == "-LOCAL-":
			if edit==False: # I still don't know how this works. I was tried 
				if level == 0:
					...
					elif values["-LOCAL-"][0] == localMain[2] and level == 0 and exists(playerLoc + "/Music"):
			...
			if edit == True and level > 0:			
				...
				if exists(playerLoc + "/Music"):
					...
					ViewAllCondense([], ViewAll(home + "playCache"))						
				level=0
				isCache=False
				vAll=False
				window["-LOCAL-"].update(values=localMain)
